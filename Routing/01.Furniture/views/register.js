...
const formTempl = (onSubmit, invalidEmail, invalidPass, invalidRePass) => html`
...
export async function registerPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        if (email == '' || password == '' || rePass == '') {
            ctx.render(formTempl(onSubmit, email == '', password == '', rePass == ''));
            return alert('All fields are recuired!');
        }
        if (password != rePass) {
            ctx.render(formTempl(onSubmit, false, true, true));
